"""Optimized axis search utilities — single string-filter pass, pixel Radon scoring."""
...
def healpix_direction_grid(dir_nside: int) -> np.ndarray:
    """Unit vectors for all pixels at *dir_nside*."""
    import healpy as hp
...
def search_best_axis(
    ...
    *,
    ...
    coarse_scores = np.array(
        [_radon_anisotropy_score(prepared.raw, d) for d in directions],
        dtype=float,
    )
    ...
    refine_scores = score_axes_batch(prepared, candidates, n_eta=max(search_n_eta, 32))
