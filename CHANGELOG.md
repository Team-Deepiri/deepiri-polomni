...
- **Multi-survey scar consensus** — `polomni data scar-consensus` / `GET /cosmos/scar-consensus`: three gates (WMAP K/Q/V axis agreement, catalog S_RBLE at CMB consensus above footprint null, residual nematic near CMB). RA-strip SDSS fetch so galaxies are all-sky. Dipole agreement is *not* a detection gate.
