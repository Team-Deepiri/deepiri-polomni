...
- **Neural guidance in the closed loop** — `ChoicePolicy.NEURAL` loads Graph-NODE axis/branch checkpoints and steers bifurcation weights (`neural/guidance.py`). Train with `polomni neural train --with-scar`; compare with `polomni neural evaluate`. Scar classifier now trains on tomogram fingerprints and is used by `polomni scan --neural` when checkpoints exist.
- **History rewrite beats uniform** — `polomni neural rewrite-history` rebuilds loop corpus labeled by the frozen hierarchical preferred axis from cached WMAP; open-loop neural pre-shifts the imprint site to the Graph-NODE axis and beats uniform by ~84° on M2 (`open_neural` 4.6° vs `open_uniform` 89°). Scar↔hierarchical transfer ~4.4°.
- **Universe multi-map scan** — `polomni scan --universe` fetches/scans WMAP K/Q/V + Planck SMICA; catalog adds `wmap_q_band`/`wmap_v_band` (NASA LAMBDA) and a working SDSS SkyServer query; Planck intensity mask cached for clean-sky scoring. Cross-sky now parses live SkyServer JSON. Honest status: WMAP bands share one hierarchical axis at nside=32; Planck differs; exoplanet↔SDSS dipoles still disagree (~79°) — interaction/compute evidence, **not** a peer-reviewed multiverse detection.
- **Multi-survey scar consensus** — `polomni data scar-consensus` / `GET /cosmos/scar-consensus`: three gates (WMAP K/Q/V axis agreement, catalog S_RBLE at CMB consensus above footprint null, residual nematic near CMB). RA-strip SDSS fetch so galaxies are all-sky. Dipole agreement is *not* a detection gate. **Frame fix:** catalogs rotated equatorial→Galactic before comparison (WMAP/Planck are Galactic-native); exoplanet residual↔CMB dropped ~84°→~14°.
- **M2 neural real-sky probe** — `polomni neural probe` runs uniform / axis_biased / neural physics loops against cached WMAP/Planck and tests whether Graph-NODE guidance improves sim↔real axis alignment (`docs/studies/M2_NEURAL_REAL_SKY_PROBE_PREREG.md`). Computational interaction evidence — does not reopen falsified P1.
