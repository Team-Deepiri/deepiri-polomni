...
Optional integration between **deepiri-polomni** (RBLE conductance network) and **optional quantum engines** (quantum entanglement simulation).
...
**optional quantum engines** supplies density matrices $\rho_i, \rho_j$ and interaction Hamiltonians for entangled branches. The bridge maps these to $\mathcal{G}_{ij}$ on the district graph.
...
This adds a path dependency on `optional quantum engines` (configure path in `pyproject.toml` to match your local clone).

Verify:

```bash
poetry run python -c "from polomni.bridge.entanglement import cross_branch_gradient; print('bridge package OK')"
```
...
| File | Function | Role |
|------|----------|------|
| `src/polomni/bridge/entanglement.py` | `cross_branch_gradient(rho_i, rho_j, H_interaction=None)` | $\mathcal{G}_{\text{interact}} = \langle \Psi_i | \hat{H} | \Psi_j \rangle$ |
| `src/polomni/bridge/er_epr_coupling.py` | `map_density_to_conductance(rho_i, rho_j, S_euclidean=1.0)` | $\rho \to \mathcal{G}_{ij}$ |
| `src/polomni/bridge/README.md` | — | Package-level notes |
...
from polomni.core.conductance.bridge_tensor import bridge_conductance
...
When `optional quantum engines` is absent:
